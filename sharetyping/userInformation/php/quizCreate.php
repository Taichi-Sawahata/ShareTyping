...
if(is_uploaded_file($tmp_path)){
   ...
   try{
     ...
     $stmt = $db->prepare('INSERT INTO quiz(user_name,title,category,q_num,datepost,content,image)
       ...
       $stmt->bindValue(':q_num',$num,PDO::PARAM_INT);
   ...
   try{
    ...
    $db->setAttribute(PDO::ATTR_EMULATE_PREPARES,false);
   $stmt = $db->prepare ('CREATE TABLE '.$title.'  (
      question VARCHAR(20),
      answer  VARCHAR(20)
   )engine=innodb default charset=utf8');
  $stmt->execute();
  for($i=0;$i<=$num-1;$i++){
    $question = htmlspecialchars($_POST['question'."$i"], ENT_QUOTES, "UTF-8");
    $answer = htmlspecialchars($_POST['answer'."$i"], ENT_QUOTES, "UTF-8");
  $stmt = $db->prepare('INSERT INTO '.$title.'(question,answer) 
        ...
        <div class="containers">
            ...
            <div class="All">
                ...
                <form action="" method="post" id="submit" enctype="multipart/form-data">
                    ...
                    <div class="Df">
                        <div class="title">
                            <div class="start"><span class="hissu">※</span><span class="font2">タイトル</span></div>
                            <div class="insert">
                                <input type="text" class="count" name="title" id="input" autocomplete="off" required>
                            </div>
                        </div>
                        <div class="category">
                            <div class="start"><span class="hissu hissuCate">※</span><span class="font2">カテゴリ</span>
                            </div>
                            <div class="insert"><input type="text" class="count" value="#" name="category" id="input"
                                    required></div>
                        </div>
                    </div>
                    <div class="Df">
                        <div class="content">
                            <div class="start"><span class="hissu">※</span><span class="font2 contents2">コンテンツ内容</span>
                            </div>
                            <div><textarea name="content" id="textarea" rows="2"></textarea></div>
                        </div>
